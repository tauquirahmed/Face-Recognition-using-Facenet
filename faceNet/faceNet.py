...
class FaceNet:
    ...
    def __init__(
        ...
        providers = (
            providers
            if ort.get_device() == "GPU" and not force_cpu
            else providers[::-1]
        )
    ...
    def l2_normalize(
        self, x: np.ndarray, axis: int = -1, epsilon: float = 1e-10
    ) -> np.ndarray:
    ...
    def detect_save_faces(
        self, image: np.ndarray, output_dir: str = "faces", name: str = "person"
    ):
        ...
        """
        face_crops = [
            image[t:b, l:r] for t, l, b, r in self.detector(image, return_tlbr=True)
        ]
    ...
    def encode(self, face_image: np.ndarray) -> np.ndarray:
        ...
        encode = self.ort_sess.run(
            None, {self.ort_sess._inputs_meta[0].name: np.expand_dims(face, axis=0)}
        )[0][0]
    ...
    def cosine_distance(
        self, a: np.ndarray, b: typing.Union[np.ndarray, list]
    ) -> np.ndarray:
